...
def check_whisper():
    ...
    try:
        ...
        print(
            "  - macOS (faster): brew install ffmpeg uv; uv venv .venv; source .venv/bin/activate; uv pip install mlx-whisper",
            file=sys.stderr,
        )
        print(
            "    (Ask your AI what the above does. At the moment pipx, python 3.14, and mlx-whisper conflict, but uv works - 11/2025)",
            file=sys.stderr,
        )
...
def check_audio_size(youtube_url):
    ...
    result = subprocess.run(["yt-dlp", "--print", "%(filesize,filesize_approx)s %(duration)s", youtube_url], capture_output=True, text=True)
...
def download_audio(youtube_url, audio_file):
    ...
    result = subprocess.run(["yt-dlp", "-x", "--audio-format", "mp3", "--output", audio_file, youtube_url], capture_output=True, text=True)
...
def get_video_language(youtube_url):
    ...
    result = subprocess.run(["yt-dlp", "--print", "%(language)s", youtube_url], capture_output=True, text=True)
...
def transcribe_with_whisper(audio_file, output_dir, mlx_command=None, quality="default"):
    ...
    if mlx_command:
        ...
        print("Transcribing with MLX Whisper (Apple Silicon optimized, large-v3 model) - this may take a while...")
        command = mlx_command
    ...
    result = subprocess.run(
        command + [audio_file, "--model", model, output_format_arg, "vtt", output_dir_arg, output_dir], capture_output=True, text=True
    )
